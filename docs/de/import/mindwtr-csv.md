...
| `Status` | `inbox`, `next`, `waiting`, `someday`, `reference`, `done`, `archived` | Unabhängig von Groß- und Kleinschreibung. Bleibt das Feld leer, wird der Status zu `archived`, wenn `Cancelled At` gesetzt ist, sonst zu `done`, wenn `Completed At` gesetzt ist, sonst zu `next`, wenn die Zeile ein Projekt nennt, andernfalls zu `inbox`. Ein nicht erkannter Wert wird mit einer Warnung zu `inbox`. |
...
| `Cancelled At` | ein Datum mit Uhrzeit | Der Zeitpunkt des Abbruchs. Bei leerem `Status` wird `archived` gewählt, bevor `Completed At` berücksichtigt wird. Der Wert bleibt nur bei `archived` erhalten; ein Abbruch löscht `Completed At` und erzeugt keine nächste Wiederholung. |
...
- Sie finden den CSV-Export auf dem Desktop bei den Aktionen für die Mehrfachauswahl. Filtern Sie eine Aufgabenliste oder die Kontextansicht, klicken Sie auf **Auswählen**, markieren Sie die gewünschten Aufgaben und wählen Sie **Ausgewählte Aufgaben als CSV exportieren**. Nur markierte Aufgaben werden exportiert, auch außerhalb des sichtbaren Ausschnitts. Für die gesamte gefilterte Liste klappen Sie zuerst die gewünschten Gruppen auf und wählen dann alle Aufgaben aus. Der Export verändert weder Aufgaben noch Auswahl. Der vollständige Export bleibt in den Einstellungen.
