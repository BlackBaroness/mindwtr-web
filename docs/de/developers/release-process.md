...
| Apple Watch | Watch-fähiges iOS-Archiv in TestFlight | Stabile Releases reichen dasselbe Watch-fähige iOS-Archiv zur App-Store-Prüfung ein und verteilen es über TestFlight. |
...
Zur Wiederherstellung eines Store-Flights nach einem veröffentlichten Release akzeptiert `.github/workflows/release-msstore-flight.yml` einen veröffentlichten stabilen oder RC-Tag. Der Workflow prüft das vorhandene portable Windows-Artefakt anhand des GitHub-SHA256-Digests und verpackt die unveränderte ausführbare Datei. Die Flight-Einreichung verlangt, dass der Vorabversionsstatus des Releases zum Tag passt; öffentliche Release-Artefakte werden nicht ersetzt und die App wird nicht neu gebaut.
...
Der wiederverwendbare iOS-Workflow trennt die Watch-Einbindung (`include_watch`) vom reinen TestFlight-Routing (`testflight_only`). Für Ad-hoc-Aufrufer sind beide Eingaben standardmäßig false. Der RC-Workflow aktiviert beide und erzeugt so einen Watch-fähigen TestFlight-Build, ohne die Production-Version im App Store zu ändern. Der stabile Workflow aktiviert Watch und lässt das reine TestFlight-Routing deaktiviert, sodass ein einziges Archiv zur App-Store-Prüfung eingereicht und über TestFlight verteilt wird. Für eine manuelle stabile Wiederherstellung wird `run_ios_appstore` ausgewählt; es gibt keinen separaten Watch-Upload-Job.
...
| T-3 | GitHub-Prerelease aus `release-rc.yml` bestätigen, Flathub-Beta-PR und die AUR-Aktualisierung von `mindwtr-beta-bin` prüfen, wenn diese Workflow-Eingaben aktiviert waren, und den RC für Tester ankündigen. |
