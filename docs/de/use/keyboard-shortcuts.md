...
- Verwenden Sie `gi` für den Eingang, `gn` für „Nächste Aktionen“ und `gf` für „Fokus“ (im Standard- und Vim-Modus).
...
- In Projekte gelangen Sie mit → von der Hauptseitenleiste in die Projektliste, wählen mit ↑/↓ ein Projekt und wechseln mit → zu dessen Aufgaben. Im Vim-Modus verwenden Sie `l`, `j`/`k` und erneut `l`; `h` führt zur Hauptseitenleiste zurück. Auch mit Tab erreichen Sie die Projektzeilen, selbst in schmalen Fenstern.
...
- Im Kontextmenü über Rechtsklick / `Shift+F10`: ↑/↓ wechselt zwischen Einträgen, → öffnet ein Untermenü und ← kehrt zurück, `Enter` aktiviert – in einem Untermenü-Panel speichert `Enter` den Wert und schließt das Menü; `Esc` schließt das Menü und setzt den Fokus zurück auf die Aufgabe.
