...
在使用 `npx` 的 MCP 配置中，将 `"--allow-scripts=better-sqlite3"` 添加到 `args` 的 `"mindwtr-mcp"` 之前。对于已安装到项目中的包，请在该项目内先运行 `npm install-scripts approve better-sqlite3`，再运行 `npm rebuild better-sqlite3`。批准前请检查该依赖，不要启用所有依赖脚本。
...
通过 Cloud 模式的 MCP 更新链接附件需要 Mindwtr Cloud 1.2.8 或更高版本。辅助程序会使用条件写入，保留其他客户端修改过的文件附件，并在发生并发修改时最多尝试三次。如果服务器没有提供记录的 `ETag`，请先升级服务器再更新链接；辅助程序会拒绝更新，不执行写入。仅修改其他字段以及创建任务或项目的行为保持不变。

这不是被搁置的托管多租户连接器。Cloud 服务器和 MCP 辅助程序仍由你自行运行；Mindwtr 并未运营存储所有人任务数据的服务。
...
_注意：请将数据库路径替换为你实际的本地 Mindwtr 数据库路径。_
...
只有当你希望 Codex 修改本地 Mindwtr 数据时，才添加 `--write`：
...
- `recurrence`：重复规则对象或 RFC 5545 RRULE 字符串
- `contexts`：string[]
- `tags`：string[]
- `description`：string
...
- 使用 `null` 清除可为空的字段。这适用于 `projectId`、`dueDate`、`startTime`、`contexts` 和 `tags` 等任务字段；`areaId`、`startDate`、`dueDate`、`reviewAt` 和 `supportNotes` 等项目字段；分区的 `description`；领域的 `color` 和 `icon`；以及人员的 `note` 和 `referenceLink`。
...
Task 和 Project 的 `cancelledAt` 是带时区的可选 ISO 时间戳。设置它会取消并归档该记录。取消的任务没有 `completedAt`，也不会生成下一次重复任务；取消项目会保留已完成步骤，取消剩余待办。同步使用此字段前，请升级所有会写入数据的客户端。详见 [GTD 工作流](/zh-Hans/use/gtd-workflow)。
