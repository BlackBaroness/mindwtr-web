...
- **周视图**：用于浏览多日规划区间的网格，桌面端默认显示七天，移动端默认显示五天。
- **月视图**：通过标记概览截止日期、已排期任务和事件。
- **日程视图**：未来 60 天的滚动列表，显示即将到来的截止日期、已排期任务和外部事件。

周视图可显示两到七天。桌面端可用标题栏中的控件调整天数，移动端可在时间线上上下滑动调整。减少天数后，其余各列会变宽，便于阅读较长的标题，也便于将任务拖放到具体时间。在桌面端，较短视图会形成连续滚动的日期范围：向前或向后导航时会按当前可见天数移动，因此不会遗漏任何日期。移动端仍保留整周，并在其中滚动较窄的窗口。Mindwtr 会按设备保存这项布局选择，不会同步。

### 回看已完成的工作

日历默认只显示尚未完成的工作。日历标题栏中的 **已完成** 开关还会显示已完成和已归档的任务，按它们完成的那一天摆放，而不是按它们可能仍带有的开始日期或截止日期。它们以灰显加删除线呈现，因此绝不会被误看成待办工作，也无法拖动到别的日期——完成是已发生之事的记录，不是计划。旁边的 **开始日期** 开关控制任务是否显示在其开始日期上——关闭后，日历只显示截止日期。

在 Mindwtr 开始记录完成时间之前归档的任务，会回退到它们的最后修改日期，与归档列表为它们显示的日期一致。

该开关是按设备保存的视图选择，不会同步；它也绝不会影响已发布的[日历订阅源](/zh-Hans/data-sync/self-hosted-cloud)——该订阅源始终只包含未完成的工作。
...
- 如果为重复任务启用了**在日历中显示未来重复**，日历会显示可见范围内每一次发生的仅供规划预览。这些预览不是真实任务；移动端/macOS 日历推送仍只把下一次发生写为普通的单次事件，而不是原生重复事件。
...
订阅的 `.ics` 源常常包含多个逻辑日历，靠每个事件的 `CATEGORIES` 字段区分。Mindwtr 会自动拆分这类日历源：每个类别在日历视图中成为独立日历，拥有各自的颜色和显示/隐藏开关。事件若列出多个类别，归入第一个。把 `CATEGORIES` 当作自由标签使用的源不会被拆分——超过八个不同类别时，该源仍是单个日历。无论是否拆分，该源在设置中都只保留一行，因为它的 URL 和移除按钮都在那里。

### 日历颜色

声明了自身颜色的日历源会保留这些颜色：Mindwtr 会读取标准的 `COLOR` 属性、Apple 的日历颜色，以及按类别设置的颜色（例如 Fossify 的），因此订阅的日历看起来和它来源应用中一样。你在 Mindwtr 中自己选择的颜色始终优先于源提供的颜色；没有任何颜色信息的日历会获得自动分配且固定不变的颜色。如需恢复日历源的颜色，请在订阅的颜色选择器中选择 **自动**。
...
Linux 不会显示单独的日历权限对话框。只读日历可以显示在 Mindwtr 中，但不会列为推送目标。Flathub 版本支持此集成。Snap 仍不受支持。
