...
| --- | --- | --- | --- | --- | --- |
| **文件同步**（文件夹） | 原生桌面端/移动端 | 低，选择一个文件夹 | 最快（普通文件系统） | 文件级；文件夹提供商看到一个文件 | Syncthing、现有云盘客户端、局域网 |
| **WebDAV** | 原生应用；允许 CORS 的浏览器/PWA | 中等，服务器 URL + 凭据 | 较慢，每次请求都需 HTTP 往返 | 应用控制、逐项合并 | Nextcloud/ownCloud/Fastmail、远程自带存储 |
| **Mindwtr Cloud（自托管）** | 原生应用 + 浏览器/PWA | 较高，部署 `apps/cloud` + 令牌 | 快，单端点，服务器写入时合并 | 应用 + 服务器端合并 | 多台设备并发编辑 |
| **Dropbox** | 支持的原生构建 | 低，OAuth 登录 | 中等，提供商 API | 应用控制、逐项合并 | 无需自建服务器的最简云端方案 |
...
原生应用以本地 SQLite 数据库作为数据源。浏览器/PWA 版本则把本地数据集保存在与来源绑定的浏览器存储中；清除网站数据或更换网站来源后，将无法访问该本地副本。浏览器不支持文件同步。只有服务器的 CORS 策略允许 PWA 来源时，浏览器 WebDAV 才能工作。

基于文件的同步使用可读的 JSON，并以文件形式保存附件，因此应把所选文件夹、服务商账户或服务器视为可信存储。Mindwtr 不运营托管同步服务。Dropbox 直连请求不经过 Mindwtr 开发者，Dropbox 令牌也只保存在你的设备上。
...
Flatpak 版本运行在沙盒中，自身不申请任何文件系统权限。当你选择同步文件夹时，桌面的文件门户只授予该文件夹的访问权限，Mindwtr 显示的路径可能类似 `/run/user/1000/doc/abcd/Mindwtr`。

- 该路径是门户为你所选文件夹建立的映射，并不是另一个 Mindwtr 数据目录。
- 请让 Syncthing 等同步工具指向你最初选择的那个文件夹。它们看不到门户路径，那只存在于 Mindwtr 的沙盒内部。

请确认 `data.json` 出现在你选择的文件夹中。如果没有出现，说明门户授权在你的系统上没有生效，可以作为备用方案给 Mindwtr 直接访问某个文件夹的权限：

```bash
flatpak override --user --filesystem="$HOME/Mindwtr" tech.dongdongbh.mindwtr
```

之后重启 Mindwtr。这是排查故障的手段，而不是常规配置——门户才是预期的方式，所以如果你需要这样做，请反馈给我。
...
后端同步失败后，自动重试会先等待约一分钟；如果连续失败，等待时间会增加到最多十分钟。服务提供商指定的重试等待时间可以覆盖这一安排。手动同步仍可使用。

### 手机端同步与电池

在 Android 和 iOS 上，上述触发方式有两点不同。离开应用时 Mindwtr 会同步一次；返回应用时，如果距上次同步已超过 30 秒，会再同步一次。对于 WebDAV、自托管的 Mindwtr Cloud、Dropbox 和 iCloud，应用还会注册一个计划后台任务，最短间隔为 15 分钟，应用关闭时也可运行；实际运行时间由操作系统决定；文件同步从不注册该任务，只在应用打开时同步。每次自动同步之后，应用至少要等待本次同步耗时的九倍，才会开始下一次（最短 5 秒，最长 5 分钟），因此数据量大时只是同步得更少，而不是持续不断地同步。

一次同步的开销随你的数据量和改动量而增加。同步加密不是耗电因素：加密或解密文档每次只需几毫秒，而刻意设计得很慢的密钥派生只在你设置、更改或输入密码短语时才会运行。

如果同步仍然比你希望的更耗电：

- **限制后台使用**：在 Android 电池设置中对 Mindwtr 限制后台使用。这只会停止计划任务；应用打开时的同步不受影响。这对你的数据是安全的：每次同步都是一次完整的读取、合并与写入，被系统中断的同步只会在下次打开应用时从头重新运行。
- **改用文件同步**：搭配你自己选择的同步应用，前提是你不需要 Mindwtr 本身在后台同步。

在 1.2.8 之后的下一版本中，后台任务将使用统一的自动调度，并移除间隔选项。升级后再次打开应用时，之前保存的所有间隔（包括关闭）都会改为同一自动调度。完全关闭同步仍会禁用后台同步。操作系统可能延迟或跳过计划任务；手动同步和应用打开时的同步仍然可用。
...
- **外观**（主题）
- **语言与日期格式**
- **GTD 偏好设置**（默认计划时间、“专注”任务上限和任务编辑器布局）
- **外部日历 URL**（ICS 订阅）
- **AI 设置**（模型/提供商）
- **已保存的筛选器**（“专注”筛选器预设）
...
- 附件在元数据合并**之后**同步。
- 缺失的附件在下载前保留为占位符。
- Mindwtr 会清理孤立附件的本地副本和元数据。桌面端也可在**设置 → 数据**中启动清理。
- WebDAV 和其他支持版本化删除的后端也能删除远程对象。清理只使用当前设备已知的引用，不会进行全局引用计数。删除共享附件前请先让设备同步；如果清理移除了另一台设备仍需要的远程副本，请重新附加该文件。
- 文件同步会清除孤立元数据和本地记录，但会把不可变的附件版本保留在共享文件夹中。进度较慢的设备可能仍引用其中的文件，因此 Mindwtr 无法证明自动删除是安全的。该文件夹可能会随时间增长。

如需释放文件同步空间，请先在文件系统中复制整个共享同步文件夹（包括 `data.json` 和 `attachments/`），并将副本存放在同步服务之外。然后让所有设备完成同步。只删除已经确认没有设备需要的附件版本。删除 Mindwtr 数据文档、锁文件或无法识别的版本可能导致数据丢失。
...
Mindwtr 会在开始同步前检查服务器的写入安全性。服务器必须为 `data.json` 返回强 ETag，并在创建、替换和删除时遵守 `If-None-Match` 与 `If-Match` 条件。没有已记录安全验证的 WebDAV 设置会在同步前接受相同检查。如果检查失败，Mindwtr 会停止同步并报告服务器不兼容。请先改用支持这些条件的服务器或配置，再重新尝试。
...
移动端现在会自动同步：
- 应用进入后台时
- 数据更改后，根据后端进行防抖（远程：首次更改等待 2 秒，连续更改等待 5 秒；文件：分别等待 8 秒和 15 秒），并根据近期同步耗时调整节奏
- 返回应用时：远程后端已超过 30 秒，或文件同步已超过 45 秒
...
如需可读或便于迁移的任务导出，请在**设置 → 数据**中展开**备份**，然后选择：

- **导出 Mindwtr CSV**：生成适合电子表格的任务文件；Mindwtr 可安全重新导入，并跳过已有任务 ID。
- **导出 TaskNotes**：生成 ZIP，其中每项任务对应一个带 YAML 前置元数据的 Markdown 文件，可直接用于 Obsidian TaskNotes 插件，也兼容 Mindwtr 的 Obsidian 导入。

平台支持时，导出、恢复和导入会创建恢复快照。导入、恢复或合并完成后，可在结果消息中轻点**撤销**立即回滚。
...
使用**设置 → 数据 → 导入数据 → 导入 Mindwtr CSV**，可导入文档规定的任务表格或包含 CSV 文件的 ZIP。格式涵盖状态、项目、分区、领域、情境、标签、负责人、优先级、精力、开始/截止/回顾日期、完成状态、清单、重复规则和手动顺序。重新导入 Mindwtr 导出时，已有任务 ID 会被跳过，不会重复或覆盖。
...
文件同步、WebDAV 和 Dropbox 可以用你自己设定的密码短语来保护。Mindwtr 写入同步位置的一切——数据文件、它的备份与恢复快照，以及附件——都会在上传或写入之前在你的设备上加密，采用 AES-256-GCM，密钥由你的密码短语经 Argon2id 派生。加密后的文件名中带有 `.enc` 标记；附件保持原有文件名，内容为加密字节。合并行为以及各设备在应用中看到的内容都不变。

在**设置 → 同步 → 加密**中开启；选中上述三种后端之一时才会出现该项。同一处也可以更改密码短语或重新关闭加密。这些操作都会重写同步位置中的全部内容，中断后可从上次的位置继续。也可以在首次同步之前就设置密码短语——密钥会保存在设备上，首次同步上传的内容从一开始就是加密的。

开启之前请先阅读：

- **WebDAV 需要服务器强制执行条件写入。** 启用前，Mindwtr 会检查服务器是否返回强 ETag，并拒绝前置条件不满足的创建或替换请求。仍然接受此类写入的服务器无法承载加密同步，在这些服务器上启用会被拒绝；明文同步照常工作。已知无法通过此检查的服务器：Fastmail、Koofr、坚果云、dufs。Nextcloud 会强制执行。
- **先更新所有设备。** 不支持加密的版本无法读取已加密的同步位置。它们的本地数据不会受损，设备更新并解锁后即可正常合并回来。
- **密码短语无法找回。** 请抄写下来或存入密码管理器。一旦遗失，同步副本将永远无法再读取；已经在你设备上的数据仍可正常读取。
- **服务商保留的历史版本维持原样。** Dropbox、你的 WebDAV 服务器或文件同步工具在加密之前保留的版本历史，仍可用旧密钥读取，或本就是明文。
- **服务商仍能看到文件夹的外形**：文件数量、大致大小以及修改时间。

尚未获得密码短语的设备会暂停自动同步并询问一次；若拒绝，同步会一直暂停到输入为止。密码短语不正确时只会再次询问，绝不会改动已存储的文件；无法读取的文件也绝不会被“修复”或删除。

同步加密不涵盖自托管的 Mindwtr Cloud、iCloud/CloudKit，以及各设备上应用自身的本地数据库。

如果你运行自己的服务器，又不希望它能读取你的任务，可以在自托管的 Mindwtr Cloud 和开启同步加密的 WebDAV 服务器之间选择。Mindwtr Cloud 会读取任务数据来合并、捕获、发布日历订阅并提供 MCP；加密 WebDAV 让服务器看不到内容，但放弃这些附加功能。

| | 自托管 Mindwtr Cloud | 开启同步加密的 WebDAV |
| --- | :---: | :---: |
| 多设备同步 | 是 | 是 |
| 附件 | 是 | 是 |
| 服务器无法读取任务 | 否 | 是 |
...
在同步加密覆盖不到的地方，静态数据的保护取决于持有数据的设备、服务器或服务商：

- **设备端**：数据以 SQLite 形式存放在应用的私有存储中。请使用操作系统级加密——FileVault（macOS）、BitLocker（Windows）、LUKS（Linux）或 iOS 和 Android 的标准设备加密——来保护本地副本。
- **iCloud（CloudKit）**：同步记录在传输中和苹果服务器上均已加密，但密钥由苹果管理。Mindwtr 未使用 CloudKit 的加密字段 API，因此任务字段不是端到端加密的，启用高级数据保护也不会改变这一点。
- **未启用同步加密的 Dropbox 和 WebDAV**：传输使用 TLS，服务商通常也会加密其存储，但密钥在服务商手中，技术上可以读取同步文档。
- **自托管云端**：隐私取决于服务器的运维方式。VPN 只保护访问和传输；要保护磁盘上的副本，仍需对服务器磁盘加密（例如 LUKS）。

若某个后端不在同步加密的覆盖范围内，或部分设备仍在旧版本上，可将文件夹同步指向由其他工具加密的文件夹——加密文件系统或 gocryptfs/Cryptomator 挂载——或搭配 Syncthing 的不受信任设备模式，让中转节点只保存密文（Syncthing 的测试版功能；受信任设备仍保留可读副本）。
