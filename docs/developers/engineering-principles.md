...
- **Update checks:** a user asked for a "disable update checks" toggle because the app kept offering GitHub downloads to a package-manager install. The fix was not the toggle: the app now detects its install channel and behaves correctly per channel — reminders pinned to channels with a canonical feed, fully quiet for channels that manage their own updates. No setting shipped.
...
- **#833:** completed focus sessions credit their minutes to the linked task automatically — manual entry exists as the correction path, not the workflow. And when a behavior genuinely cannot be inferred and demand is real, gate it behind an *existing* switch before minting a new one: task time tracking appears only while the existing Pomodoro timer and its task linking are enabled; no new setting was added.

**Guardrails:** before adding a setting, a prompt, or a manual workflow step, prove the outcome cannot be inferred or predicted. Before minting a new setting or affordance, prove no existing one already expresses the same intent. Defaults follow platform best practice, not neutrality. Manual controls are the fallback for correction, never the primary path when automation is possible.
