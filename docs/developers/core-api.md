...
The snippets below show commonly used fields and are not exhaustive. Import the exported types from `@mindwtr/core`, or use `packages/core/src/types.ts` as the authoritative definition.
...
interface Task {
    ...
    timeEstimate?: TimeEstimate;   // Presets through '4hr+', or `custom:${number}` minutes
    ...
    orderNum?: number;             // Legacy alias accepted from older payloads
}
```
...
interface Recurrence {
    rule: RecurrenceRule;
    seriesId?: string;                   // Stable recurring-series identity
...
- Clients must also preserve `seriesId` and any anchor metadata when round-tripping recurrence objects.
...
interface AppData {
    ...
    settings: {
        ...
        language?: 'en' | 'vi' | 'zh' | 'zh-Hant' | 'es' | 'hi' | 'ar' | 'de' | 'ru' | 'ja' | 'fr' | 'pt' | 'pl' | 'ko' | 'cs' | 'it' | 'tr' | 'nl' | 'fa' | 'sv' | 'hu' | 'uk' | 'system';
...
| `%Person`    | `%Jim` or `%"Jim Smith"` | `assignedTo: 'Jim'` (known names match unquoted multi-word via `knownPeople`) |
| `/due:date`  | `/due:friday`      | `dueDate: 'ISO string'`     |
| `/energy:<level>` | `/energy:high` | `energyLevel: 'high'` (supports `low`, `medium`, `high`) |
| `/priority:<level>` | `/priority:high` | `priority: 'high'` (supports `low`, `medium`, `high`, `urgent`) |
| `/note:text` | `/note:remember X` | `description: 'remember X'` |
| `/status`    | `/next`            | `status: 'next'` (supports `/inbox`, `/waiting`, `/someday`, `/reference`, `/done`, `/archived`) |
