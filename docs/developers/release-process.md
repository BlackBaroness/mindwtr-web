...
For a Store flight recovery after a release is published, `.github/workflows/release-msstore-flight.yml` accepts a published stable or RC tag. It verifies the existing Windows portable asset against GitHub's SHA256 digest and packages that unchanged executable. The flight submission path requires an RC or stable release whose prerelease state matches the tag; it does not replace public release assets or rebuild the app.
...
| T-3 | Confirm the GitHub prerelease from `release-rc.yml`, verify the Flathub beta PR and the `mindwtr-beta-bin` AUR update when those workflow inputs were enabled, and announce the RC to testers. |
