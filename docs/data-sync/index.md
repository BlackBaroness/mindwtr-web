...
This page is the user-facing setup and recovery guide for the docs site. For maintainer-level merge rules and diagnostics fields, see [Sync Algorithm](/data-sync/sync-algorithm).

For desktop vault import and note deep links, see [Obsidian Integration](/power-users/obsidian).
...
- **Guide**: [iCloud Sync](/data-sync/icloud)
...
- **Docker setup**: [Docker Deployment](/power-users/docker-deployment)
- **Operations guide**: [Cloud Deployment](/data-sync/cloud-deployment)
...
- Use HTTPS for custom DNS, VPN hostnames, Tailscale, ZeroTier, and any name that is not recognized as local/private, or enable **Allow insecure connections (HTTP)** in the sync settings to accept that hostname over plain HTTP. Data then travels unencrypted, so only use it on a network you trust.
...
- **Guide**: [Dropbox Sync](/data-sync/dropbox)
...
If sync still uses more battery than you want:
...
See [ADR 0009](https://github.com/dongdongbh/Mindwtr/blob/main/docs/adr/0009-sqlite-json-sync-bridge.md) for the full contract.
...
See [Backup and Restore](/data-sync/backup-restore) for the detailed flow.
...
See [TickTick Import](/import/ticktick) for details and supported mappings.
...
See [Todoist Import](/import/todoist) for details and supported mappings.
...
See [DGT GTD Import](/import/dgt-gtd) for details and supported mappings.
...
See [OmniFocus Import](/import/omnifocus) for details and supported mappings.
