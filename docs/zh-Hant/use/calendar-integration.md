...
- **週檢視**：用於瀏覽多日規劃區間的格線，桌面版預設顯示七天，行動版預設顯示五天。
- **月檢視**：以標記呈現截止期限、已排程任務與事件的概覽。
- **行程檢視**：未來 60 天的截止期限、已排程任務與外部事件清單。

週檢視可顯示兩到七天。桌面版可使用標題列中的控制項調整天數，行動版可在時間軸上向上或向下滑動調整。減少天數後，其餘各欄會變寬，方便閱讀較長的標題，也方便將任務拖放到特定時間。在桌面版，較短檢視會形成連續捲動的日期範圍：向前或向後瀏覽時會依目前顯示的天數移動，因此不會遺漏任何日期。行動版仍會保留整週，並在其中捲動較窄的視窗。Mindwtr 會依裝置保存這項版面配置選擇，不會同步。

### 回顧已完成的工作

行事曆預設只顯示尚未完成的工作。行事曆標題列中的 **已完成** 開關還會顯示已完成與已封存的任務，依它們完成的那一天擺放，而不是依它們可能仍帶有的開始日期或截止日期。它們以淡化加刪除線呈現，因此絕不會被誤看成待辦工作，也無法拖曳到其他日期——完成是已發生之事的紀錄，不是計畫。旁邊的 **開始日期** 開關控制任務是否顯示在其開始日期上——關閉後，行事曆只顯示截止日期。

在 Mindwtr 開始記錄完成時間之前封存的任務，會回退到它們的最後修改日期，與封存清單為它們顯示的日期一致。

此開關是依裝置保存的檢視選擇，不會同步；它也絕不會影響已發布的[行事曆訂閱來源](/zh-Hant/data-sync/self-hosted-cloud)——該訂閱來源始終只包含未完成的工作。
...
- 若為重複任務啟用**在行事曆顯示未來重複**，行事曆會顯示可見範圍內每一次發生的僅供規劃預覽。這些預覽並非真正的任務；行動版／macOS 行事曆推送仍只將下一次發生寫成一般單一事件，而不是原生重複事件。
...
訂閱的 `.ics` 來源常包含多個邏輯行事曆，靠每個事件的 `CATEGORIES` 欄位區分。Mindwtr 會自動拆分這類來源：每個類別在行事曆檢視中成為獨立行事曆，擁有各自的顏色與顯示／隱藏開關。事件若列出多個類別，歸入第一個。把 `CATEGORIES` 當作自由標籤使用的來源不會被拆分——超過八個不同類別時，該來源仍是單一行事曆。無論是否拆分，該來源在設定中都只保留一列，因為它的網址與移除按鈕都在那裡。

### 行事曆顏色

宣告自身顏色的來源會保留這些顏色：Mindwtr 會讀取標準的 `COLOR` 屬性、Apple 的行事曆顏色，以及依類別設定的顏色（例如 Fossify 的），因此訂閱的行事曆看起來與它來源 app 中一樣。你在 Mindwtr 中自行選擇的顏色一律優先於來源提供的顏色；沒有任何顏色資訊的行事曆會取得自動指派且固定不變的顏色。如需恢復行事曆來源的顏色，請在訂閱的顏色選擇器中選擇 **自動**。
...
5. 返回 Mindwtr 的**選單 → 設定 → 進階 → 行事曆**，點選**重新整理行事曆**，再選擇顯示你 Google 帳號的 `Mindwtr` 目標。
...
3. 在 Mindwtr 中開啟**設定 → 整合 → 行事曆**。
4. 若要將 Mindwtr 任務寫入系統行事曆，請啟用**將任務推送至行事曆**。
5. 展開**同步目標**，重新整理行事曆，然後選擇專用的 `Mindwtr` 行事曆或其他可寫入目標。

Linux 不會顯示獨立的行事曆權限對話框。唯讀行事曆可顯示在 Mindwtr 中，但不會列為推送目標。Flathub 版本支援此整合。Snap 仍不受支援。
