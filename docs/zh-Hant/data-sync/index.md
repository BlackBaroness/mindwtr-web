...
[開啟互動式圖表](/assets/diagrams/sync-setup.html){target="_self"}
...
在 Android 與 iOS 上，上述觸發方式有兩點不同。離開應用程式時 Mindwtr 會同步一次；返回應用程式時，若距上次同步已超過 30 秒，會再同步一次。對於 WebDAV、自架的 Mindwtr Cloud、Dropbox 與 iCloud，應用程式還會註冊一個排程背景工作，最短間隔為 15 分鐘，應用程式關閉時也可執行；實際執行時間由作業系統決定；檔案同步從不註冊該工作，只在應用程式開啟時同步。每次自動同步之後，應用程式至少要等待這次同步耗時的九倍，才會開始下一次（最短 5 秒，最長 5 分鐘），因此資料量大時只是同步得更少，而不是持續不斷地同步。
...
在 1.2.8 之後的下一版本中，背景工作將使用統一的自動排程，並移除間隔選項。升級後再次開啟應用程式時，先前儲存的所有間隔（包括關閉）都會改為同一自動排程。完全關閉同步仍會停用背景同步。作業系統可能延遲或略過排程工作；手動同步和應用程式開啟時的同步仍然可用。
