...
| T-3 | 確認由 `release-rc.yml` 建立的 GitHub 預發布版本；當對應工作流程輸入已啟用時，驗證 Flathub beta PR 與 `mindwtr-beta-bin` 的 AUR 更新；並向測試人員公告 RC。 |
...
整個區塊的前提是儲存庫為 `dongdongbh/Mindwtr`、儲存庫變數 `SIGNPATH_SIGNING_ENABLED` 設為 `true`、兩個 SignPath 祕密都已設定，且存在標籤。分支儲存庫、提取請求以及沒有標籤的手動執行，依設計會建置為未簽署版本。該變數是總開關：未設定時簽署步驟會被靜默略過，發佈將以未簽署版本發出，且沒有任何失敗檢查予以提示。
