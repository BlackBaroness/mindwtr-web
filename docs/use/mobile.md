...
2. Select **Widgets**
3. Find **Mindwtr** and add the **Tasks** widget, the one-cell **Quick capture** button or, on Android, the optional smaller **Compact** widget. The Tasks widget shows a **+** button and your tasks, each with its priority colour, its project or area, and its due date. On Android, a newly added Tasks widget shows Focus, with the inbox count in its header and a short view of Today's Focus and Today. Review Due, Next actions and Upcoming stay in the app. Tap the title to open that list in Mindwtr. To switch the widget's list, tap the separate small down arrow. The header follows the same theme background as the widget body. A sheet lists Focus, Inbox, Next Actions, Waiting For, Someday/Maybe and your **Saved Filters**. Tap one and the widget redraws. To show a single project on a widget, save a Focus filter for that project in the app and pick that filter here. This works on every launcher, and some launchers have no edit action of their own for widgets. You can add several Tasks widgets with different lists. After Mindwtr has run and published its first Android widget snapshot, the five standard lists are ready for newly added Tasks widgets and list changes without another app launch. Open Mindwtr once before using widgets and again to refresh their data; widgets do not follow background sync in real time. Mindwtr publishes saved-filter snapshots on demand.
...
The Android Compact widget uses smaller text and a flat Focus list for small widget spaces. It shows Today's Focus and Today under the **Today** header. If both are empty and Next Actions are available, the header switches to **Next Actions**. Resizing the widget shows as many task rows as fit, up to the available tasks. It has no list chooser or inline completion control; tapping a task opens the same native detail sheet.
...
| `/priority:<level>` | `/priority:high` | Sets priority (`low`, `medium`, `high`, `urgent`) |
...
See [Attachments](/use/attachments) for details on syncing and cleanup.
...
Mindwtr can overlay external calendars and push dated Mindwtr tasks to the device calendar. Detailed setup lives in [Calendar Integration](/use/calendar-integration).
...
For Android Google Calendar and iOS Apple Calendar target setup, see [Calendar Integration](/use/calendar-integration).
