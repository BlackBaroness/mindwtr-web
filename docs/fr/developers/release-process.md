...
| Apple Watch | Archive iOS avec Watch dans TestFlight | Les versions stables soumettent la même archive iOS avec Watch à la vérification de l’App Store et la distribuent via TestFlight. |
...
| Arch Linux | AUR `mindwtr-beta-bin` | La version stable actualise le paquet bêta persistant. |
| Linux Debian/Fedora | Dépôts APT/RPM bêta | Les paquets stables restent dans des répertoires de dépôt stable distincts. |
| Téléchargement direct Windows | Programme d'installation/version portable en préversion GitHub | La version GitHub finale fournit les téléchargements stables. |
| Windows Microsoft Store | Package flight Mindwtr Beta | Les versions stables actualisent le flight configuré et la soumission publique du Store. |
...
Les flights Microsoft Store utilisent la variable du dépôt `MSSTORE_FLIGHT_ID` et les secrets existants `MS_TENANT_ID`, `MS_CLIENT_ID`, `MS_CLIENT_SECRET` et, facultativement, `MS_STORE_APP_ID`. L’ID d’application par défaut est `9N0V5B0B6FRX`. Exécutez `.github/workflows/msstore-flight-id.yml` pour obtenir l’ID du flight. Les testeurs inscrits reçoivent les mises à jour depuis la fiche habituelle du Store.

L’entrée RC `run_msstore_flight` est activée par défaut. Pour les RC, le flux Windows conserve `run_msstore=false` et utilise uniquement l’API des soumissions de flight. Il refuse les brouillons en attente qui ne lui appartiennent pas, y compris un premier brouillon créé dans Partner Center ; examinez-le et publiez-le ou abandonnez-le avant de réessayer. Ne modifiez pas dans Partner Center les soumissions créées par l’API. Le résumé du flux indique l’ID de soumission et l’état du traitement ; la certification et l’installation par les testeurs sont vérifiées séparément.

Pour reprendre un flight Store après la publication d’une version, `.github/workflows/release-msstore-flight.yml` accepte une étiquette stable ou RC déjà publiée. Le workflow vérifie l’archive Windows portable existante avec l’empreinte SHA256 de GitHub et empaquette l’exécutable inchangé. La soumission au flight exige que l’état de préversion corresponde à l’étiquette ; elle ne remplace pas les ressources publiques et ne reconstruit pas l’application.
...
Le flux de travail iOS réutilisable sépare l’inclusion de la Watch (`include_watch`) du routage réservé à TestFlight (`testflight_only`). Ces deux entrées valent false par défaut pour les appels ponctuels. Le flux RC active les deux et produit ainsi un build avec Watch réservé à TestFlight, sans modifier la version de production de l’App Store. Le flux stable active la Watch et désactive le routage réservé à TestFlight : une seule archive est soumise à la vérification de l’App Store et distribuée via TestFlight. Une récupération stable manuelle sélectionne `run_ios_appstore` ; il n’existe plus de tâche distincte pour téléverser la Watch.
...
| T-3 | Confirmez la préversion GitHub créée par `release-rc.yml`, vérifiez la pull request Flathub bêta et la mise à jour AUR de `mindwtr-beta-bin` lorsque ces entrées étaient activées, puis annoncez la RC aux testeurs. |
