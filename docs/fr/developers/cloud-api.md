...
À la création, `props` accepte ces champs de tâche : `status`, `projectId`, `sectionId`, `areaId`, `description`, `priority`, `dueDate`, `startTime`, `relativeStartOffset`, `reviewAt`, `recurrence`, `showFutureRecurrence`, `contexts`, `tags`, `checklist`, `attachments`, `assignedTo`, `location`, `energyLevel`, `timeEstimate`, `timeSpentMinutes`, `taskMode`, `textDirection`, `isFocusedToday`, `pushCount`, `repeatReminderMinutes`, `suppressMindwtrReminders` et `viewSectionIds`. `title` est un champ de premier niveau, pas une prop. La modification accepte les mêmes champs plus `title`, `order`, `orderNum`, `boardOrder` et `focusOrder`. Les champs gérés par le serveur, comme `id`, `createdAt`, `updatedAt`, `completedAt`, `deletedAt` et `rev`, ne sont jamais modifiables par le client. Les listes proviennent de l’indicateur `cloudWrite` dans `packages/core/src/task-sync-schema.ts` ; ce fichier est donc la référence actuelle.
...
- `POST /v1/capture-tokens` crée un jeton de capture seule pour le compte. Le corps est `{ "label": "Pebble ring" }`, et l'étiquette est facultative. Il renvoie `201` avec `id`, `token`, `label` et `createdAt`. Le jeton est affiché une seule fois.
- `GET /v1/capture-tokens` liste les jetons de capture seule du compte avec `id`, `label` et `createdAt`. Aucun secret.
- `DELETE /v1/capture-tokens/:id` en révoque un.

Les trois exigent le jeton bearer complet. Un jeton de capture seule n'est accepté que sur `POST /v1/capture` et est rejeté avec `403` sur toute autre route.
