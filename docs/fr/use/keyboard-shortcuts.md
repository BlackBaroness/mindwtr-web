...
- Dans Projets, utilisez → depuis la barre latérale principale pour accéder à la liste des projets, ↑/↓ pour choisir un projet, puis → pour accéder à ses tâches. En mode Vim, utilisez `l`, `j`/`k`, puis `l` ; `h` revient à la barre latérale principale. Tab permet aussi d’atteindre les projets, même dans une fenêtre étroite.
