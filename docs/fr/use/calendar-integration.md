...
- Si **Voir les occurrences à venir dans le calendrier** est activé pour une tâche récurrente, le calendrier affiche des aperçus de planification de chaque occurrence dans la plage visible. Ces aperçus ne sont pas de véritables tâches, et l’envoi vers le calendrier sur mobile/macOS n’inscrit toujours que la prochaine occurrence comme un événement unique normal, et non comme un événement récurrent natif.
...
Linux n’affiche pas de boîte de dialogue d’autorisation distincte pour le calendrier. Les calendriers en lecture seule peuvent apparaître dans Mindwtr, mais ne sont pas proposés comme cibles d’envoi. La version Flathub prend en charge cette intégration. Snap reste non pris en charge.
