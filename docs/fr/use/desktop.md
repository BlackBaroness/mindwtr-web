...
- **Terminer** : utilisez **Terminer** dans le menu **...** de l’en-tête du projet une fois le résultat atteint — les tâches restantes sont terminées avec lui, le projet passe dans la section **Terminés** en bas de la barre latérale, et **Réactiver** annule cette action
...
Tâches terminées qui ont été classées. Les tâches archivées sont masquées des listes de tâches normales, mais vous pouvez les rechercher ou les restaurer ici. Cliquez sur le titre d’une tâche pour ouvrir ses notes, sous-tâches et pièces jointes en lecture seule, sans la restaurer. Cliquez sur l’horodatage **Terminée** pour le corriger, ou utilisez l’action de restauration de la ligne pour renvoyer la tâche dans la Boîte de réception. Un sélecteur Tâches | Projets en haut affiche à la place les projets archivés : restaurer un projet le réactive (en récupérant les tâches terminées avec lui), et le supprimer l’envoie à la corbeille. Cliquez sur le nom d’un projet archivé pour l’ouvrir dans Projets, en lecture seule, avec les tâches terminées avec lui.

Archivés partage la rangée de commandes des autres listes de tâches : **Filtres** restreint par contexte, étiquette, priorité et durée estimée, tandis que **Trier** et **Grouper** proposent tous deux la **date d’achèvement**, puisque tout ici est du travail terminé. Par défaut, les achèvements les plus récents apparaissent en premier. Les sélections de filtres sont partagées entre les listes de tâches : un contexte choisi ailleurs restreint donc aussi cette liste.
