...
![Configurer la synchronisation : choisissez un backend dans les Réglages, saisissez le dossier, l'URL du serveur ou la connexion, puis appuyez sur Enregistrer, ce qui teste la destination. WebDAV demande en plus un test d'écriture et la phrase secrète est facultative. Les branches latérales montrent une configuration refusée, une synchronisation en pause et une nouvelle tentative.](/assets/diagrams/sync-setup.svg)

[Ouvrir le diagramme interactif](/assets/diagrams/sync-setup.html){target="_self"}
...
WebDAV utilise HTTPS pour les URL publiques. HTTP en clair est automatiquement autorisé pour les cibles locales/privées reconnues telles que `localhost`, `127.0.0.1`, `10.x.x.x`, de `172.16.x.x` à `172.31.x.x`, `192.168.x.x`, les adresses IPv6 de bouclage/privées, `*.local` et `*.home.arpa`. Pour un DNS personnalisé, les noms d’hôte de VPN, Tailscale, ZeroTier et tout nom qui n’est pas reconnu comme local/privé, utilisez HTTPS ou activez **Autoriser HTTP non sécurisé (non recommandé)** dans les réglages de synchronisation. Les données circulent alors en clair ; n’utilisez cette option que sur un réseau de confiance.
...
- **Préférences GTD** (heure de planification par défaut, limite de tâches dans Focus et disposition de l’éditeur de tâches)
...
Pour libérer de l’espace dans le dossier de synchronisation de fichiers, faites d’abord une copie complète du dossier partagé dans le système de fichiers, y compris `data.json` et `attachments/`, puis conservez cette copie hors du service de synchronisation. Laissez ensuite tous les appareils terminer leur synchronisation. Supprimez uniquement les générations de pièces jointes dont vous avez vérifié qu’aucun appareil n’a besoin. La suppression du document de données Mindwtr, des fichiers de verrouillage ou d’une génération que vous ne pouvez pas identifier peut entraîner une perte de données.
