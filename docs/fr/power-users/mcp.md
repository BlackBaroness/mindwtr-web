...
Dans les configurations MCP utilisant `npx`, ajoutez `"--allow-scripts=better-sqlite3"` avant `"mindwtr-mcp"` dans `args`. Pour une installation locale existante, exécutez `npm install-scripts approve better-sqlite3`, puis `npm rebuild better-sqlite3` depuis ce projet. Vérifiez la dépendance avant de l’autoriser ; n’activez pas tous les scripts.
...
Antigravity (l'IDE agentique de Google) lit les serveurs MCP locaux dans un fichier de configuration JSON ; il n'existe pas de commande CLI pour en ajouter. Ouvrez le fichier depuis le panneau latéral de l'agent via **MCP Servers** -> **Manage MCP Servers** -> **View raw config**, ou modifiez-le directement :

- **Versions récentes :** `~/.gemini/config/mcp_config.json` (par espace de travail : `.agents/mcp_config.json`)
- **Versions plus anciennes :** `~/.gemini/antigravity/mcp_config.json` (Windows : `%USERPROFILE%\.gemini\antigravity\mcp_config.json`)
...
Remplacez le chemin par celui de votre base de données locale (les chemins Windows exigent des antislashs doublés en JSON). Omettez `--write` pour un accès en lecture seule. Redémarrez Antigravity et vérifiez que `mindwtr` apparaît dans ses paramètres de serveurs MCP. La configuration MCP locale d'Antigravity est indépendante du `settings.json` de Gemini CLI et des apps personnalisées de l'application Gemini.
...
- **`mindwtr_add_project`** : créer un projet, notamment avec les champs facultatifs `startDate`, `dueDate` et `reviewAt`. Accepte aussi les pièces jointes de type lien via `attachments`.
- **`mindwtr_update_project`** : mettre à jour un projet, notamment avec les champs facultatifs `startDate`, `dueDate` et `reviewAt`. Les pièces jointes de type lien dans `attachments` suivent la même règle de liste complète que lors de la mise à jour d’une tâche.
...
- Autres champs modifiables : `areaId`, `reviewAt`, `taskMode`, `relativeStartOffset`, `showFutureRecurrence`, `pushCount`, `checklist`, `textDirection`, `location`, `isFocusedToday`, `timeSpentMinutes`, `suppressMindwtrReminders`, `repeatReminderMinutes`, `attachments`
...
- **Accès concurrent (mode SQLite local) :** le serveur utilise le mode WAL de SQLite et relance l’écriture avec les données actuelles après un conflit de verrouillage temporaire. Le client ne doit réessayer que si ces tentatives échouent.
