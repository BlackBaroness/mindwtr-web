...
| Apple Watch | Archivo de iOS con Watch en TestFlight | Las versiones estables envían el mismo archivo de iOS con Watch a revisión de App Store y lo distribuyen mediante TestFlight. |
...
Para recuperar un piloto de Store después de publicar una versión, `.github/workflows/release-msstore-flight.yml` acepta una etiqueta estable o RC publicada. Verifica el archivo portátil de Windows existente con el resumen SHA256 de GitHub y empaqueta el ejecutable sin cambios. El envío al piloto exige que el estado de prelanzamiento coincida con la etiqueta; no sustituye los archivos públicos de la versión ni recompila la aplicación.
...
El flujo reutilizable de iOS separa la inclusión de Watch (`include_watch`) del envío exclusivo a TestFlight (`testflight_only`). Ambas entradas son false de forma predeterminada para ejecuciones ad hoc. El flujo de RC activa ambas y genera una compilación con Watch solo para TestFlight, sin cambiar la versión de producción de App Store. El flujo estable activa Watch y desactiva el envío exclusivo a TestFlight, por lo que un único archivo se envía a revisión de App Store y se distribuye mediante TestFlight. La recuperación estable manual selecciona `run_ios_appstore`; no hay un trabajo separado para subir Watch.
...
| T-3 | Confirma el prelanzamiento de GitHub de `release-rc.yml`, verifica la solicitud de incorporación de cambios de la beta de Flathub y la actualización de AUR `mindwtr-beta-bin` cuando esas entradas estuvieran habilitadas, y anuncia la RC a quienes vayan a probarla. |
