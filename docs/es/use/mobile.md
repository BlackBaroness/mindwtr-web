...
3. Busca **Mindwtr** y añade el widget **Tareas**, el botón de una celda **Captura rápida** o, en Android, el widget opcional más pequeño **Compacto**. El widget Tareas muestra un botón **+** y tus tareas, cada una con su color de prioridad, su proyecto o área y su fecha de vencimiento. En Android, un widget Tareas recién añadido muestra Foco, con el recuento de la bandeja de entrada en su encabezado y una vista breve de Foco de hoy y Hoy. Revisión pendiente, Próximas acciones y Próximas se quedan en la aplicación. Toca el título para abrir esa lista en Mindwtr. Para cambiar la lista del widget, toca la pequeña flecha hacia abajo, que es un control independiente. El encabezado usa el mismo fondo del tema que el cuerpo del widget. Se abre una lista con Foco, Bandeja de entrada, Próximas acciones, En espera, Algún día/Tal vez y tus **filtros guardados**. Toca uno y el widget se vuelve a dibujar. Para mostrar un solo proyecto en un widget, guarda en la aplicación un filtro de Foco para ese proyecto y elígelo aquí. Esto funciona en cualquier lanzador, y algunos lanzadores no ofrecen ninguna acción de edición propia para los widgets. Puedes añadir varios widgets Tareas con listas distintas. Después de ejecutar Mindwtr y publicar la primera instantánea de widgets de Android, las cinco listas estándar quedan disponibles para nuevos widgets Tareas y para cambios de lista sin volver a abrir la aplicación. Abre Mindwtr antes de usar los widgets por primera vez y de nuevo cuando quieras actualizar sus datos; los widgets no reflejan la sincronización en segundo plano en tiempo real. Mindwtr publica las instantáneas de los filtros guardados bajo demanda.
...
El widget Compacto de Android usa texto más pequeño y una lista plana de Foco para espacios pequeños. Muestra Foco de hoy y Hoy bajo el encabezado **Hoy**. Si ambos están vacíos y hay Próximas acciones, el encabezado cambia a **Próximas acciones**. Al cambiar el tamaño del widget, se muestran tantas tareas como quepan, hasta agotar las disponibles. No tiene selector de lista ni control de finalización en línea; al tocar una tarea se abre la misma ficha nativa.

En Android, tocar una fila de tarea en Tareas o Compacto abre una pequeña ficha de tarea sobre la pantalla de inicio, sin abrir la aplicación principal. La ficha muestra el título, el proyecto o área, la nota, los contextos y etiquetas, las fechas de inicio y de vencimiento, y la prioridad. **Completar** marca la tarea igual que el anillo de Tareas, con la misma breve ventana para deshacer. **Abrir** te lleva a la tarea en la aplicación.

En Android, tocar el anillo del widget Tareas tacha la fila de inmediato. Tócalo de nuevo en unos tres segundos para deshacer. Después, la fila desaparece de Tareas y Compacto y deja espacio para las tareas restantes, sin abrir la aplicación. La finalización queda guardada en el dispositivo hasta que vuelvas a abrir Mindwtr, que la aplica y la sincroniza con tus otros dispositivos.
...
En iOS 17 o posterior, mantén pulsado el widget y elige **Editar widget** para seleccionar Enfoque, Bandeja de entrada, Próximas acciones, En espera, Algún día/Tal vez o un filtro guardado. Varios widgets pueden mostrar listas distintas; una lista elegida distinta de Enfoque nunca cambia automáticamente a otra. Toca un anillo para dejar pendiente la finalización y vuelve a tocarlo en tres segundos para deshacer. Después, la fila se oculta y otra tarea guardada en caché puede ocupar su lugar. WidgetKit puede retrasar la actualización visible. La finalización queda guardada en la cola hasta que Mindwtr se ejecute y la guarde; la finalización y la sincronización se realizan desde la app.

Toca el título, el estado vacío o el widget pequeño para abrir la lista mostrada. En los widgets medianos y grandes, tocar una tarea la abre en Mindwtr y **+** abre la captura rápida; los anillos del widget pequeño funcionan desde iOS 17. Las versiones anteriores usan el comportamiento predeterminado Hoy/Próximas sin completar tareas ni configurar la lista desde el widget. WidgetKit controla cuándo se actualiza. Los widgets de la pantalla bloqueada no cambian.
...
| `/priority:<level>` | `/priority:high` | Establece la prioridad (`low`, `medium`, `high`, `urgent`) |
